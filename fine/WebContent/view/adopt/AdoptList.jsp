...
<body>
	...
	<table border=1>
		...
		<tr>
			<td colspan="6"><a href="./view/adopt/AdoptWrite.jsp">글작성</a></td>
		</tr>
	</table>
	<!--페이징 아래 숫자-->
	<div class="training_content_side">
		...
		%>
		<div> <!-- 이전 -->
			<a href="<%=ctxpath%>/adoptList.do?pageNum=<%=startPage%>"> 이전</a>
			<%
				} else {
			%>
		...
		<div> <!-- 다음 -->
			...
			%>
			<a href="<%=ctxpath%>/adoptList.do?pageNum=<%=endPage%>"> 다음</a>
			<%
					} else {
			%>
			<a href="<%=ctxpath%>/adoptList.do?pageNum=<%=currentPage + 1%>">
