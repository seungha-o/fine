...
<script>
	function goInsertComment() {
		...
		frm.action = "<%=ctxPath%>/adoptReply.do";
		frm.method = "get";
		frm.submit();
	}
</script>
...
<body>
	...
	<section>
		...
		<c:if test="${not empty detailList }">
			<table id="writeTable">
				<tr>
					<td class="td1"><input type="hidden" name=adopt_no
						value="${detailList.adopt_no}">번호</td>
				...
				<tr>
					<td class="td1">이미지</td>
					<td><c:forEach items="${detailList.adopt_img}" var="img">
			...
			<form name="commentFrm" id="commentFrm">
				...
				<c:if test="${not empty list }">
					<c:forEach items="${list }" var="mvo" step="1">
						<div class="comments">
							<p>${mvo.id }</p>
							<p> : ${mvo.adopt_contents }</p>
								<c:if test="${id eq member_id }">
									<button type="button" id="BtnDetail" onclick="window.location='<%=ctxPath%>/adoptDelete.do?adopt_no=${mvo.adopt_no}'">삭제</button>
								</c:if> <br>
						</div>
						<input type="hidden" name=ref_step value="${mvo.ref_step }">
						<input type="hidden" name=ref_level value="${mvo.ref_level }">
					</c:forEach>
				</c:if>
			</form>
			</c:if>
			<div id="bottom">
					...
					<c:if test="${id eq member_id }">
							<a href="adoptDetailToUpdate.do?adopt_no=${detailList.adopt_no}"
							class="btn">수정</a>
							<a href="adoptDelete.do?adopt_no=${detailList.adopt_no}" class="btn">삭제</a>
					</c:if>
			</div>
	</section>
	<!-- FOOTER -->
