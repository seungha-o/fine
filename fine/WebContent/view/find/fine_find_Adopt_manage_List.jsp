...
<body>
	...
	<c:forEach items="${list }" var="mvo" varStatus="s">
		<tr>
		<td><img src="./upload/${mvo.filename }" style="width: 50px; height: 50px"></td>
		...
		<td>${mvo.sexCd }</td>
		<td>${mvo.neuterYn }</td>
		<td><button type="button" id="BtnDetail" onclick="window.location='<%=ctxPath%>/find_manage_delete.do?no=${mvo.desertionNo}'">삭제</button></td>
		</tr>
	...
	<h1>찾는 결과가 없습니다.</h1>
		...
		<button type="button" id="BtnWrite" onclick="window.location='<%=ctxPath %>/view/find/fine_find_manage_insert.jsp'">글쓰기</button>
	<%
