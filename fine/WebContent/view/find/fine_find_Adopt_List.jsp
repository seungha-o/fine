...
<div id="map" style="width:800px;height:500px;"></div>
	...
	<%
		...
		int pagecount = (int)request.getAttribute("pagecount");
			if (count > 0) {
				...
				%>
				[<a href="./find_adopt_List.do?pageNum=<%=currentPage - 1%>">이전</a>]
				<%
			}
		...
		<a href="<%=ctxPath%>/find_adopt_List.do?pageNum=<%=i%>"><%=i%></a>
	...
	%>
		[<a style="display=none" href="<%=ctxPath%>/find_adopt_List.do?pageNum=<%=currentPage + 1%>">다음</a>]
	<%
		} else {
		%>
		[<a href="<%=ctxPath%>/find_adopt_List.do?pageNum=<%=currentPage + 1%>">다음</a>]
		<%
			}
