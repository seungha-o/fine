...
<body>
	...
	<section>
			...
			<c:if test="${ not empty qnaList }">
				<c:forEach items="${ qnaList }" var="list" varStatus="s">
					<tr>
						<td>${s.count}</td>
						<td>${list.id}</td>
						<td><a href="qnaDetailCheck.do?qna_no=${list.qna_no}">${list.qna_title}</a> <!-- Update -->
						</td>
		...
		<c:if test="${not empty level }">
			<div id="bottom">
				<a href="./view/qna/QnAWrite.jsp" class="btn">글작성</a>
		...
		<div class="paging">
			...
			<div id="paging">
				<div> <!-- 이전 -->
					<a href="<%=ctxPath%>/qnaList.do?pageNum=<%=startPage%>"><button class="btn"><span>[이전]</span></button></a>
					<%
						} else {
					%>
				...
				<div> <!-- 다음 -->
					...
					%>
					<a href="<%=ctxPath%>/qnaList.do?pageNum=<%=endPage%>"><button class="btn"><span>[다음]</span></button></a>
					<%
							} else {
					%>
					<a href="<%=ctxPath%>/qnaList.do?pageNum=<%=currentPage + 1%>">
