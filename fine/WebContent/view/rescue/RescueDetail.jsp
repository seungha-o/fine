...
<script>
	function goInsertComment() {
		...
		frm.action = "<%=ctxPath%>/rescueReply.do";
		frm.method = "get";
		frm.submit();
	}
</script>
<title>rec 상세페이지</title>
<style type="text/css">
img{
	object-fit: cover;
}
	...
	<h2>Rescue 상세보기</h2>
		<c:if test="${not empty detailList }">
			<table id="writeTable">
				<tr>
					<td class="td1">번호
					<input type="hidden" name=rec_no value="${detailList.rec_no}"></td>
			...
			<form name="commentFrm" id="commentFrm">
				...
				<c:if test="${not empty list }">
					<c:forEach items="${list }" var="mvo" step="1">
						<div class="comments">
							<p>${mvo.id }</p>
							<p> : ${mvo.rec_contents }</p>
							<c:if test="${id eq member_id }">
								<button type="button" id="BtnDetail" onclick="window.location='<%=ctxPath%>/rescueDelete.do?rec_no=${mvo.rec_no}'">삭제</button>
							</c:if> <br>
						</div>						
		...
		<div id="bottom">
			...
			<c:if test="${id eq member_id }">
				<a href="rescueDetailToUpdate.do?rec_no=${detailList.rec_no}" class="btn">수정</a>
				<a href="rescueDelete.do?rec_no=${detailList.rec_no}" class="btn">삭제</a>
			</c:if>
		</div>
	</section>
	<!-- FOOTER -->
