...
public class Fine_MyPage_SearchId extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    
    public Fine_MyPage_SearchId() {
        super();
...
//		(String)request.getSession().getAttribute("memberLev");
		...
		try {
			...
			if (currentPage % pageBlock == 0) {
				...
				endPage = startPage + pageBlock - 1;
		            ...
		            List<MemberVO> searchPageList = services.SearchId(startRnum , endRnum, searchId); // list에 뿌려줄 data를 다 받아왔음
		...
		try {
			...
			if(lev.equals("3")) {
					...
					request.setAttribute("myPageList", searchPageList); 
					RequestDispatcher dispatcher = request.getRequestDispatcher("./view/member/myPageAdminSearch.jsp");
